...
import com.ethan.floatmenudemo.floatball.runner.ICarrier;
import com.ethan.floatmenudemo.floatball.runner.ScrollRunner;
 ...
 * 子菜单项布局
 */
public class FloatMenuLayout extends ViewGroup implements ICarrier {
    ...
    public FloatMenuLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        MIN_RADIUS = DensityUtil.dip2px(context, 80);
     ...
     * 计算半径
     */
    private static int computeRadius(final float arcDegrees, final int childCount, final int childSize, final int childPadding, final int
            minRadius) {
        if (childCount < 2) {
            return minRadius;
        }
//        final float_ball perDegrees = arcDegrees / (childCount - 1);
...
//        final float_ball perDegrees =Math.abs (mToDegrees - mFromDegrees) / (childCount - 1);
        float perDegrees;
